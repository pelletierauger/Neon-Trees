...
smoothLine.fragText = `
    ...
    void main(void) {
        ...
        col = min(col * -1. * (1. / radius), 1.);
        col = pow(col, 3.) * 0.75 + pow(col, 23.);
        ...
        // if (col < 0.001) {
            // discard;
        // }
        gl_FragColor = vec4(c.rgb, 0.25 * c.a * col - (rando * 0.005));
        // gl_FragColor.g = pow(col, 2.) *  0.2;
        // gl_FragColor.b = pow(col, 2.) *  0.2;
        
        // gl_FragColor.b += 0.15;
        gl_FragColor.rgb = vec3(0., 0.0, 0.);
...
smoothDots.vertText = `
    ...
    void main(void) {
        vec2 pos = coordinates.xy;
        pos += vec2(
            cos(pos.x*pos.y*4.+time*0.05), 
            sin(pos.x*pos.y*4.+time*0.05))*0.05;
        // pos += vec2(
        //     cos(pos.x*pos.y*400.+time*1.1*sign(pos.x*pos.y*400.)), 
        //     sin(pos.x*pos.y*400.+time*1.1*sign(pos.x*pos.y*400.)))*0.0025;
        // pos.x *= resolution.y / resolution.x;
...
smoothDots.fragText = `
    ...
    void main(void) {
        ...
        float noise = rand(pos - vec2(cos(t), sin(t))) * 0.0625;
        gl_FragColor = vec4(vec3(1.0, pow(l, 2.)*0.25, 0.25), l+halo-noise);
        gl_FragColor.a *= (sin(-t*0.1+posUnit.y*1e-2)*0.5+0.5);
        gl_FragColor.a *= (sin(-t*0.25+(posUnit.x*78.+posUnit.y*43.))*0.5+0.5);
    }
    // endGLSL
`;
smoothDots.vertText = smoothDots.vertText.replace(/[^\x00-\x7F]/g, "");
